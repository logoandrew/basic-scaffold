...
$(function () {
  ...
  //var $colorInputs = $textInputs.add("#fPick, #bPick");
  //var $colorSliders = $("#fLightness, #bLightness");
  ...
  //   var $this = $(this);
  //   var color = $this.val();
  //   var context = $this.attr("id").substr(0, 1);
  //
  ...
  //     color =
  //     "#" +
  //     color.substr(1, 1).repeat(2) +
  //     color.substr(2, 1).repeat(2) +
  //     color.substr(-1).repeat(2);
  //   $this.val(color);
  //
  //   // Validation
...
function update() {
  ...
  $(".permalink").attr(
    "href",
    "./?fcolor=" + f.substr(1) + "&bcolor=" + b.substr(1)
  );
  $(".apilink").attr(
    "href",
    "./?fcolor=" + f.substr(1) + "&bcolor=" + b.substr(1) + "&api"
  );
  $(".apilink").text(
...
function changeHue(context) {
  HSL = RGBtoHSL(
    getRGB(eval(context).substr(1, 2)),
    getRGB(eval(context).substr(3, 2)),
    getRGB(eval(context).substr(-2))
  );
...
function checkContrast() {
  ...
  $("#ratio").html(
    "<b>" + (Math.round(ratio * 100) / 100).toFixed(2) + "</b>:1"
  );
  if (ratio >= 4.5) {
    $("#normalAA, #bigAAA")
      .attr("class", "pass")
...
function getsRGB(c) {
  c = getRGB(c) / 255;
  c = c <= 0.03928 ? c / 12.92 : (c + 0.055) / Math.pow(1.055, 2.4);
  return c;
}
